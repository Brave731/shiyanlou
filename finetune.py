...
test_image_0_path = 'test/0/'  # 指定测试集数据路径（根据实际情况指定测试数据集的路径）
test_image_1_path = 'test/1/'  # 指定测试集数据路径（根据实际情况指定测试数据集的路径）

label_path = []
test_label = []

# 打开训练数据集目录，读取全部图片，生成图片路径列表
image_path = np.array(glob.glob(train_image_path + '0.*.jpg')).tolist()
image_path_1 = np.array(glob.glob(train_image_path + '1.*.jpg')).tolist()
image_path[len(image_path):len(image_path)] = image_path_1
for i in range(len(image_path)):
    if '1' in image_path[i]:
        label_path.append(1)
    else:
        label_path.append(0)

# 打开测试数据集目录，读取全部图片，生成图片路径列表
test_image = np.array(glob.glob(test_image_0_path + '*.jpg')).tolist()
_len = len(test_image )
test_image_path_1 = np.array(glob.glob(test_image_1_path + '*.jpg')).tolist()
test_image[len(test_image):len(test_image)] = test_image_path_1
